...
export const VERSION='ds-research-0.2.0';
...
export function generate(all,asOf,config,routeConfig,boundaryPoints=[]){
  ...
  function add(center,sources,kind,path=[],route=null){
    ...
    if(route)factor('Configured corridor',12,`${route.name}; initial research route, canonical map unverified`,[route.id]);
    ...
    const objections=['This research ruleset is uncalibrated; model match is not earthquake probability.','Midpoint and corridor patterns can arise by chance or ordinary aftershock clustering.','Initial route graph is illustrative and has not been validated against the canonical Dutchsinse map.'];
  ...
  if(config.rules.deep&&config.rules.routes)for(const e of deep)for(const route of routeConfig.routes){
    let index=0;let d=Infinity;route.points.forEach((p,i)=>{const km=distance(p,e);if(km<d){index=i;d=km;}});
    if(d>1400)continue;
    for(let j=index+1;j<Math.min(route.points.length,index+3);j++)add(route.points[j],[e],'deep-cascade',[e,...route.points.slice(index,j+1)],route);
    if(config.rules.reflection&&route.termination&&index===route.points.length-1)add(route.points[index-1],[e],'reflection',[e,route.points[index],route.points[index-1]],route);
  }
